...
impl Verifier {
    ...
    pub async fn make_verify_request_async<S: AsRef<str> + core::fmt::Debug>(
        client: &reqwest::Client,
        querystring: S,
    ) -> Result<u64, Error> {
        log::debug!("(SteamVerifier) Making async request for {:?}", querystring);

        let (req, verifier) = match Self::from_querystring(querystring) {
            Ok(rv) => rv,
            Err(err) => {
                log::warn!("(SteamVerifier) Failed to parse querystring: {:?}", err);
                return Err(Error::AuthenticationFailed);
            }
        };

        log::debug!("(SteamVerifier) Request: {:?}", req);

        let (parts, body) = req.into_parts();

        log::debug!("(SteamVerifier) Request parts: {:?}", parts);
        log::debug!("(SteamVerifier) Request body: {:?}", body);
